...
def main():
    ...
    for game_index, game in enumerate(all_games, start=1):
        ...
        if game_index <= PRINT_LIMIT:
            ...
            for p in players:
                print(f"  {p} - Rate: {player_stats[p]['rate']:.2f}, Games Played: {player_stats[p]['games_played']}")
            print(f"Computed Ranks: {[(p, r) for p, r in zip(players, ranks)]}")
            print(f"Table Avg Rate (post min check): {avg_rate:.2f}")
        ...
        for idx, player in enumerate(players):
            ...
            if game_index <= PRINT_LIMIT:
                ...
                print(f"  Rate Change Calc: rate_change = {rate_coef:.3f} * ({match_result:.2f} + ({avg_rate:.2f} - {player_rate:.2f})/40)")
